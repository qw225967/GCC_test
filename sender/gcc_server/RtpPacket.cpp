...
void RtpPacket::SetExtensions(uint8_t type,
  ...
  auto paddedExtensionsTotalSize = static_cast<size_t>(
      Utils::Byte::PadTo4Bytes(static_cast<uint16_t>(extensionsTotalSize)));
...
void RtpPacket::SetPayloadLength(size_t length) {
  ...
  length = static_cast<size_t>(
      Utils::Byte::PadTo4Bytes(static_cast<uint16_t>(length)));
...
//  MS_ASSERT(static_cast<size_t>(ptr - buffer) == this->size,
//            "ptr - buffer == this->size");
...
//  MS_ASSERT(payloadOffset < this->payloadLength,
//            "payload offset bigger than payload size");
