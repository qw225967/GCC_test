...
PacedSender::PacedSender(PacketRouter* packet_router,
    ...
    : packet_router_(packet_router),
      ...
      time_last_process_us_(cls::GetCurrentStamp64()),
...
void PacedSender::CreateProbeCluster(int bitrate_bps, int cluster_id) {
  ...
  prober_.CreateProbeCluster(bitrate_bps, cls::GetCurrentStamp64(), cluster_id);
}
...
int64_t PacedSender::TimeUntilNextProcess() {
  int64_t elapsed_time_us =
      cls::GetCurrentStamp64() - time_last_process_us_;
  ...
  if (prober_.IsProbing()) {
    int64_t ret = prober_.TimeUntilNextProbe(cls::GetCurrentStamp64());
...
void PacedSender::Process() {
  int64_t now_us = cls::GetCurrentStamp64();
  ...
  if (bytes_sent != 0)
  {
    auto now = cls::GetCurrentStamp64();
...
void PacedSender::OnPacketSent(size_t size) {
  if (first_sent_packet_ms_ == -1)
    first_sent_packet_ms_ = cls::GetCurrentStamp64();
